    ...
    if($registro['estado'] == 'pendiente') {
        ...
        <div class="solicitud">
            <div class="solicitud-header">
                <h3>Solicitud de Adopción de "<?= $registro['nombre'] ?>"</h3>
                <span class="solicitud-fecha">
                    <?= $fecha->format('d/m/Y') ?> · <?= $fecha->format('H:i') ?>
                </span>
            </div>
            <div class="solicitud-mensaje">
        El usuario <b><?= $registro['nombres'] ?> <?= $registro['apellido_paterno'] ?> <?= $registro['apellido_materno'] ?></b> ha enviado una solicitud de adopción de <b>"<?= $registro['nombre'] ?>"</b>.
            </div>
            <div class="acciones-formularios">
                <form action="inicio.php?pagina=adopcion" method="post" style="display:inline;">
                ...
                <form action="inicio.php?pagina=adopcion" method="post" style="display:inline;">
                    <input type="hidden" name="idDueño" value="<?= $registro['idD']; ?>">
                    <input type="hidden" name="idAnimal" value="<?= $registro['idA']; ?>">
                    <input type="hidden" name="idUsuario" value="<?= $registro['idU']; ?>">
                    <input type="hidden" name="nombreA" value="<?= $registro['nombre']; ?>">
                    <input type="hidden" name="confirmado" value="<?= $registro['id_adopcion']; ?>">
                    <button type="submit" class="btn-confirmar">Confirmar Adopción</button>
                </form>
                <form action="inicio.php?pagina=adopcion" method="post" style="display:inline;">
                    <input type="hidden" name="idUsuario" value="<?= $registro['idU']; ?>">
                    <input type="hidden" name="nombreA" value="<?= $registro['nombre']; ?>">
                    <input type="hidden" name="cancelado" value="<?= $registro['id_adopcion']; ?>">
                    <button type="submit" class="btn-cancelar">Cancelar Adopción</button>
                </form>
            </div>
        </div>
<?php }  
...
foreach ($registros as $registro) { 
    if($registro['estado'] == 'cancelado') {
        ...
        <div class="solicitudR">
            ...
            <div class="solicitud-mensaje">
        Se rechazó la solicitud de adopción de <b>"<?= $registro['nombre'] ?>"</b> enviada por el usuario <b><?= $registro['nombres'] ?> <?= $registro['apellido_paterno'] ?> <?= $registro['apellido_materno'] ?></b>.
            </div>
        </div>
<?php }  
} 
...
foreach ($registros as $registro) { 
    if($registro['estado'] == 'confirmado') {
        ...
        <div class="solicitudC">
            ...
            <div class="solicitud-mensaje">
        Se aceptó la solicitud de adopción de <b>"<?= $registro['nombre'] ?>"</b> enviada por el usuario <b><?= $registro['nombres'] ?> <?= $registro['apellido_paterno'] ?> <?= $registro['apellido_materno'] ?></b>.
            </div>
        </div>
<?php }  
}
    ...
    <div id="modalUsuario" class="modal" style="display:block;">
        <div class="modal-contenido">
            ...
            <?php foreach ($usuarios as $registro){ ?>
                <div class="reporte">
                    <p><strong>Nombre de la Cuenta de Usuario:</strong> <?= $registro['nombre_usuario'] ?></p>
...
if(isset($_POST['animal']) && $animales != NULL){ ?>
        ...
        <div class="modal-contenido">
            ...
            <?php foreach ($animales as $registro){ ?>
                <div class="reporte">
                    ...
                    <?php
                        $fecha = new DateTime($registro['fecha_nacimiento']);
                    ?>
...
if(isset($_POST['confirmado'])){ ?>
    <div id="modalConfirmado" class="modal" style="display:block;">
        <div class="modal-contenido">
            ...
            <h2>Confirmación de la adopción.</h2>
            <p>Se ha confirmado la adopción.</p>
        </div>
    </div>
<?php } 
...
if(isset($_POST['cancelado'])){ ?>
    <div id="modalCancelado" class="modal" style="display:block;">
        <div class="modal-contenido">
            ...
            <h2>Cancelación de la adopción.</h2>
            <p>Se ha cancelado la adopción.</p>
        </div>
    </div>
<?php } ?>
